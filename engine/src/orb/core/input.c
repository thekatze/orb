...
b8 orb_input_init(usize* memory_requirement, void* memory) { 
    *memory_requirement = sizeof(input_system_state);
    ...
    state = memory;
    orb_memory_zero(state, sizeof(input_system_state));
...
void orb_input_update() {
    orb_memory_copy(&state->previous_keyboard, &state->current_keyboard, sizeof(keyboard_state));
    orb_memory_copy(&state->previous_mouse, &state->current_mouse, sizeof(mouse_state));
}

// is the key down
b8 orb_input_is_key_down(orb_keyboard_keys key) { return state->current_keyboard.keys[key]; }
...
// is the key up
b8 orb_input_is_key_up(orb_keyboard_keys key) { return !state->current_keyboard.keys[key]; }
...
void orb_input_process_key(orb_keyboard_keys key, b8 pressed) {
    ...
    orb_event_send(pressed ? ORB_EVENT_KEY_PRESSED : ORB_EVENT_KEY_RELEASED, nullptr, context);
}
...
b8 orb_input_is_mouse_button_pressed(orb_mouse_buttons button) {
    return state->current_mouse.buttons[button] && !state->previous_mouse.buttons[button];
}
...
void orb_input_process_mouse_button(orb_mouse_buttons button, b8 pressed) {
    ...
    orb_event_send(pressed ? ORB_EVENT_MOUSE_BUTTON_PRESSED : ORB_EVENT_MOUSE_BUTTON_RELEASED,
                   nullptr, context);
}
