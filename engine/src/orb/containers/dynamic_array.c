...
void _orb_dynamic_array_resize(orb_dynamic_array *array) {
    ...
    void *new_items = orb_allocate(new_capacity * array->stride, MEMORY_TAG_DYNAMIC_ARRAY);
...
void _orb_dynamic_array_insert_at(orb_dynamic_array *array, usize index, const void *value_ptr) {
    ...
    if (index != array->length - 1) {
        orb_memory_copy((void *)(insert_address + array->stride), (void *)insert_address,
...
void orb_dynamic_array_remove_at(orb_dynamic_array *array, usize index, void *destination) {
    ...
    if (index != array->length - 1) {
        orb_memory_copy((void *)remove_address, (void *)(remove_address + index * array->stride),
                        array->stride * (array->length - index));
    }
