...
b8 orb_vulkan_renderpass_create(orb_vulkan_context *context,
  ...
  ORB_VK_EXPECT(vkCreateRenderPass(context->device.logical_device,
                                   &render_pass_info, context->allocator,
                                   &out_renderpass->handle));

  return TRUE;
}

void orb_vulkan_renderpass_destroy(orb_vulkan_context *context,
...
void orb_vulkan_renderpass_begin(orb_vulkan_command_buffer *command_buffer,
  ...
  VkClearValue color = {
      .color =
          {
              .float32 = {renderpass->r, renderpass->g, renderpass->b,
                          renderpass->a},
          },
  };
