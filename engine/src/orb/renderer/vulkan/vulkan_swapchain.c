...
b8 orb_vulkan_swapchain_recreate(orb_vulkan_context *context, u32 width,
                                 u32 height, orb_vulkan_swapchain *swapchain) {
  cleanup_swapchain(context, swapchain);
  return create_swapchain(context, width, height, swapchain);
}

void orb_vulkan_swapchain_shutdown(orb_vulkan_context *context,
                                   orb_vulkan_swapchain *swapchain) {
  cleanup_swapchain(context, swapchain);
}

b8 orb_vulkan_swapchain_acquire_next_image_index(
    orb_vulkan_context *context, orb_vulkan_swapchain *swapchain,
    u64 timeout_ns, VkSemaphore image_available_semaphore, VkFence fence,
    u32 *out_image_index) {
  ...
  case VK_ERROR_OUT_OF_DATE_KHR:
    ...
    return FALSE;
  default:
    ORB_FATAL("vkAcquireNextImageKHR failed with result: %s",
...
b8 orb_vulkan_swapchain_present(orb_vulkan_context *context,
                                orb_vulkan_swapchain *swapchain,
...
b8 create_swapchain(orb_vulkan_context *context, u32 width, u32 height,
  ...
  swapchain_extent.width =
      ORB_CLAMP(swapchain_extent.width, min.width, max.width);
  swapchain_extent.height =
      ORB_CLAMP(swapchain_extent.height, min.height, max.height);

  u32 desired_image_count = capabilities.minImageCount + 1;
  u32 image_count =
      capabilities.maxImageCount == 0 // 0 means unlimited, according to spec
          ? desired_image_count
  ...
  if (queueFamilyIndices[0] != queueFamilyIndices[1]) {
    ...
    swapchain_create_info.queueFamilyIndexCount =
        ORB_ARRAY_LENGTH(queueFamilyIndices);
    swapchain_create_info.pQueueFamilyIndices = queueFamilyIndices;
  }

  ORB_VK_EXPECT(vkCreateSwapchainKHR(context->device.logical_device,
                                     &swapchain_create_info, context->allocator,
                                     &out_swapchain->handle));

  context->current_frame = 0;
  out_swapchain->image_count = 0;
  ORB_VK_EXPECT(vkGetSwapchainImagesKHR(context->device.logical_device,
                                        out_swapchain->handle,
  ...
  ORB_VK_EXPECT(vkGetSwapchainImagesKHR(
  ...
  for (u32 i = 0; i < out_swapchain->image_count; ++i) {
    ...
    ORB_VK_EXPECT(vkCreateImageView(context->device.logical_device,
