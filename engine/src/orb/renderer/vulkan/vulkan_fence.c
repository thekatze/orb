...
b8 orb_vulkan_fence_create(orb_vulkan_context *context, b8 create_signalled,
    ...
    ORB_VK_EXPECT(vkCreateFence(context->device.logical_device, &fence_info, context->allocator,
                                &out_fence->handle));
    return TRUE;
}
...
b8 orb_vulkan_fence_reset(orb_vulkan_context *context, orb_vulkan_fence *fence) {
    if (fence->is_signalled) {
        ORB_VK_EXPECT(vkResetFences(context->device.logical_device, 1, &fence->handle));
