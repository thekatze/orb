...
b8 orb_vulkan_image_create(orb_vulkan_context *context, VkImageType image_type,
                           u32 width, u32 height, VkFormat format,
                           VkImageTiling tiling, VkImageUsageFlags usage_flags,
                           VkMemoryPropertyFlags memory_flags, b8 create_view,
  ...
  ORB_VK_EXPECT(vkCreateImage(context->device.logical_device,
  ...
  u32 memory_type = orb_vulkan_find_memory_index(
  ...
  ORB_VK_EXPECT(vkAllocateMemory(context->device.logical_device,
                                 &memory_allocate_info, context->allocator,
                                 &out_image->memory));
  ORB_VK_EXPECT(vkBindImageMemory(context->device.logical_device,
                                  out_image->handle, out_image->memory, 0));

  if (create_view) {
    out_image->view = 0;
    return orb_vulkan_image_view_create(context, format, out_image,
                                        view_aspect_flags);
  }
...
void orb_vulkan_image_destroy(orb_vulkan_context *context,
                              orb_vulkan_image *image) {
  if (image->view) {
