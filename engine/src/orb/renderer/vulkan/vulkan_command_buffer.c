...
b8 orb_vulkan_command_buffer_allocate(
    orb_vulkan_context *context, VkCommandPool pool, b8 is_primary,
  ...
  ORB_VK_EXPECT(vkAllocateCommandBuffers(context->device.logical_device,
                                         &allocate_info,
...
void orb_vulkan_command_buffer_free(
...
b8 orb_vulkan_command_buffer_allocate_and_begin_single_use(
    orb_vulkan_context *context, VkCommandPool pool,
    orb_vulkan_command_buffer *out_command_buffer) {
  return orb_vulkan_command_buffer_allocate(context, pool, TRUE,
                                            out_command_buffer) &&
         orb_vulkan_command_buffer_begin(
             out_command_buffer, VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT);
}

b8 orb_vulkan_command_buffer_end_and_submit_single_use(
    orb_vulkan_context *context, VkCommandPool pool,
    orb_vulkan_command_buffer *command_buffer, VkQueue queue) {
