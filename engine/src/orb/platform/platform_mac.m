...
- (void)mouseMoved:(NSEvent *)event {
  ...
  i16 x = (i16)(pos.x * state_ptr->handle.layer.contentsScale);
...
- (void)windowDidChangeScreen:(NSNotification *)notification {
  ...
  state_ptr->handle.layer.contentsScale =
      state_ptr->view.window.backingScaleFactor;
  ...
  *memory_requirement = sizeof(internal_state);
  ...
  @autoreleasepool {
    ...
    if (!state_ptr->app_delegate) {
      ORB_ERROR("Failed to create application delegate");
      return false;
    }
    ...
    if (!state_ptr->handle.layer) {
      ORB_ERROR("Failed to create layer for view");
    }
    ...
    state_ptr->handle.layer.drawableSize =
        [state_ptr->view convertSizeToBacking:state_ptr->view.bounds.size];
    ...
    // surface was created. See also
    // https://github.com/KhronosGroup/MoltenVK/issues/428
    state_ptr->handle.layer.contentsScale =
        state_ptr->view.window.backingScaleFactor;
    ORB_DEBUG("contentScale: %f", state_ptr->handle.layer.contentsScale);
    // Save off the device pixel ratio.
    state_ptr->device_pixel_ratio = state_ptr->handle.layer.contentsScale;
...
void *orb_platform_allocate(u64 size, b8 aligned) { return malloc(size); }

void orb_platform_free(void *block, b8 aligned) { free(block); }

void *orb_platform_memory_zero(void *block, u64 size) {
  return memset(block, 0, size);
}

void *orb_platform_memory_copy(void *dest, const void *source, u64 size) {
  return memcpy(dest, source, size);
}

void *orb_platform_memory_set(void *dest, i32 value, u64 size) {
  return memset(dest, value, size);
}

// FATAL,ERROR,WARN,INFO,DEBUG,TRACE
const char *colour_strings[LOG_LEVEL_MAX_ENUM] = {"0;41", "1;31", "1;33",
                                                  "1;32", "1;34", "1;30"};

void orb_platform_console_write(const char *message, u8 colour) {
  printf("\033[%sm%s\033[0m", colour_strings[colour], message);
}

void orb_platform_console_write_error(const char *message, u8 colour) {
  printf("\033[%sm%s\033[0m", colour_strings[colour], message);
}
...
f64 orb_platform_time_now(void) {
  ...
  u64 nanos = (f64)(mach_absolute * (u64)clock_timebase.numer) /
              (f64)clock_timebase.denom;
...
// b8 kthread_create(pfn_thread_start start_function_ptr, void *params,
//                   b8 auto_detach, kthread *out_thread) {
//   if (!start_function_ptr) {
//     return false;
//   }
//
//   // pthread_create uses a function pointer that returns void*, so cold-cast
//   to
//   // this type.
//   i32 result = pthread_create((pthread_t *)&out_thread->thread_id, 0,
//                               (void *(*)(void *))start_function_ptr, params);
//   if (result != 0) {
//     switch (result) {
//     case EAGAIN:
//       ORB_ERROR("Failed to create thread: insufficient resources to create "
//                 "another thread.");
//       return false;
//     case EINVAL:
//       ORB_ERROR("Failed to create thread: invalid settings were passed in "
//                 "attributes..");
//       return false;
//     default:
//       ORB_ERROR(
//           "Failed to create thread: an unhandled error has occurred.
//           errno=%i", result);
//       return false;
//     }
//   }
...
//       switch (result) {
//       case EINVAL:
//         ORB_ERROR("Failed to detach newly-created thread: thread is not a "
//                   "joinable thread.");
//         return false;
//       case ESRCH:
//         ORB_ERROR(
//             "Failed to detach newly-created thread: no thread with the id "
//             "%#x could be found.",
//             out_thread->thread_id);
//         return false;
//       default:
//         ORB_ERROR("Failed to detach newly-created thread: an unknown error
//         has "
//                   "occurred. errno=%i",
//                   result);
//         return false;
...
// }
//
// void kthread_destroy(kthread *thread) { kthread_cancel(thread); }
//
...
//       switch (result) {
//       case EINVAL:
//         ORB_ERROR("Failed to detach thread: thread is not a joinable
//         thread."); break;
//       case ESRCH:
//         ORB_ERROR("Failed to detach thread: no thread with the id %#x could
//         be "
//                   "found.",
//                   thread->thread_id);
//         break;
//       default:
//         ORB_ERROR(
//             "Failed to detach thread: an unknown error has occurred.
//             errno=%i", result);
//         break;
//       }
//     }
...
//       switch (result) {
//       case ESRCH:
//         ORB_ERROR("Failed to cancel thread: no thread with the id %#x could
//         be "
//                   "found.",
//                   thread->thread_id);
//         break;
//       default:
//         ORB_ERROR(
//             "Failed to cancel thread: an unknown error has occurred.
//             errno=%i", result);
//         break;
//       }
//     }
...
// }
//
// void kthread_sleep(kthread *thread, u64 ms) { platform_sleep(ms); }
//
// u64 platform_current_thread_id(void) { return (u64)pthread_self(); }
// NOTE: End threads.
...
//   if (result != 0) {
//     ORB_ERROR("Mutex creation failure!");
//     return false;
//   }
//
//   // Save off the mutex handle.
//   out_mutex->internal_data = platform_allocate(sizeof(pthread_mutex_t),
//   false);
...
//   if (mutex) {
//     i32 result = pthread_mutex_destroy((pthread_mutex_t
//     *)mutex->internal_data); switch (result) { case 0:
//       // KTRACE("Mutex destroyed.");
//       break;
//     case EBUSY:
//       ORB_ERROR("Unable to destroy mutex: mutex is locked or referenced.");
//       break;
//     case EINVAL:
//       ORB_ERROR(
//           "Unable to destroy mutex: the value specified by mutex is
//           invalid.");
//       break;
//     default:
//       ORB_ERROR("An handled error has occurred while destroy a mutex:
//       errno=%i",
//                 result);
//       break;
//     }
...
//     return true;
//   case EOWNERDEAD:
//     ORB_ERROR("Owning thread terminated while mutex still active.");
//     return false;
//   case EAGAIN:
//     ORB_ERROR(
//         "Unable to obtain mutex lock: the maximum number of recursive mutex "
//         "locks has been reached.");
//     return false;
//   case EBUSY:
//     ORB_ERROR("Unable to obtain mutex lock: a mutex lock already exists.");
//     return false;
//   case EDEADLK:
//     ORB_ERROR("Unable to obtain mutex lock: a mutex deadlock was detected.");
//     return false;
//   default:
//     ORB_ERROR(
//         "An handled error has occurred while obtaining a mutex lock:
//         errno=%i", result);
//     return false;
//   }
// }
...
//     i32 result = pthread_mutex_unlock((pthread_mutex_t
//     *)mutex->internal_data); switch (result) { case 0:
//       // KTRACE("Freed mutex lock.");
//       return true;
//     case EOWNERDEAD:
//       ORB_ERROR("Unable to unlock mutex: owning thread terminated while mutex
//       "
//                 "still active.");
//       return false;
//     case EPERM:
//       ORB_ERROR("Unable to unlock mutex: mutex not owned by current
//       thread."); return false;
//     default:
//       ORB_ERROR("An handled error has occurred while unlocking a mutex lock:
//       "
//                 "errno=%i",
//                 result);
//       return false;
...
// b8 platform_unwatch_file(u32 watch_id) { return unregister_watch(watch_id); }
...
//       }
//
//       // Check the file time to see if it has been changed and update/notify
//       if
//       // so.
