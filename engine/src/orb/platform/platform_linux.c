...
b8 orb_platform_init(orb_platform_state *platform, const char *application_name, i32 x, i32 y,
    ...
    xcb_change_property(state->connection, XCB_PROP_MODE_REPLACE, state->window,
                        wm_protocols_reply->atom, XCB_ATOM_ATOM, sizeof(xcb_atom_t) * 8, 1,
...
b8 orb_platform_events_pump(orb_platform_state *platform) {
    ...
    while (TRUE) {
        ...
        case XCB_KEY_RELEASE: {
            ...
            KeySym symbol =
                XkbKeycodeToKeysym(state->display, (KeyCode)code, 0, code & ShiftMask ? 1 : 0);
        ...
        case XCB_MOTION_NOTIFY: {
            xcb_motion_notify_event_t *mouse_move_event = (xcb_motion_notify_event_t *)event;

            orb_input_process_mouse_move(mouse_move_event->event_x, mouse_move_event->event_y);
        } break;
